...
- Optional **HTTP Basic Auth** (v1.6.0): set `AUTH_USER` + `AUTH_PASS` to lock the UI and API behind a credential (constant-time comparison, both variables required or the server refuses to start). Recommended for any instance reachable beyond localhost; pair it with TLS (a reverse proxy) — Basic Auth over plain HTTP is only as private as the network segment it crosses.
- **Every path that takes the admin password is throttled per client address** — the login form (v1.35.0) and, since v1.43.0, the Basic `Authorization` header, which carries the same password and was an unthrottled bypass before (measured: ~200 guesses/s while the form said 429). The two share one budget per address; session cookies and API tokens never consult it, so a logged-in admin keeps working while their address is limited. Details: after `LOGIN_MAX_ATTEMPTS` failed attempts (default 5) in a `LOGIN_WINDOW_MINUTES` window (default 15), further attempts get a `429` with `Retry-After` until the window lapses; a successful login clears the counter. The throttle keys off the socket's peer address — behind a reverse proxy that is the proxy's address (forwarded headers are attacker-writable and deliberately ignored), so it bounds the total guess rate through the proxy.
