...
services:
  lanscope:
    image: ghcr.io/dannyruizb/lanscope:1.32.0
...
- **Version detection** *(v1.32)* — how hard `-sV` works to name the product and version behind an open port. *Light* (default, `--version-light`) runs the fast probes only; *Standard* (bare `-sV`, nmap's default intensity) tries more; *All* (`--version-all`) runs **every** probe nmap has — slower, but the one that pins an odd service sitting on a non-standard port. Absent means Light, so every pre-v1.32 scan is unchanged. Applies to both the TCP and UDP flows; validated server-side to the three levels before reaching `execFile`.
...
- `POST /api/hosts/:id/portscan` shells out to `nmap (--top-ports N | -p <spec>) (-sT | -sS) -sV -T<n> (--version-light | --version-all | ) --reason [--script=default,safe] -oX - <ip>` and persists the result, including each port's `state_reason` from nmap. Defaults are `--top-ports 100 -sT -T4 --version-light`; ports selection, scan technique, timing, version-detection intensity and NSE script categories are all overridable via the *Advanced options* panel and validated server-side before reaching `execFile`. NSE output is parsed from `<port><script>` (port-level) and `<hostscript><script>` (host-level) and stored alongside the ports.
- `POST /api/hosts/:id/udp-portscan` shells out to `nmap (--top-ports N | -p <spec>) -sU -sV -T<n> (--version-light | --version-all | ) --reason -oX - <ip>`. Reuses the same ports, timing and version-detection options; scan technique does not apply (UDP-only flow). 30-minute server-side timeout to accommodate the inherent slowness of UDP scanning.
...
- [x] **v1.30.0** — **Read-only API tokens**. The scraper token from v1.28 opens the same door as the admin: if the Prometheus credential leaks, it can launch scans, drop schedules and mint more tokens. `POST /api/tokens` grows an optional `scope` — `"read"` mints a token that can **GET everything but change nothing** (the same GET/HEAD/OPTIONS trio the demo's read-only mode allows); absent or a spelled-out `"full"` stores `NULL` — one spelling of "everything", the mute-types precedent — so **every v1.25/v1.29 token keeps behaving unchanged** (`scope` is a nullable column added by soft `ALTER TABLE`). One deliberate departure from the one-failure-shape rule: a write with a read token gets a **named 403**, not the bare 401 — the bare 401 exists so *invalid* credentials have nothing to enumerate, but this caller **proved identity**, and *"your token is read-only"* is the answer to why the cron broke (the visible-expiry philosophy, applied to writes). The refused write still stamps `last_used_at`: a rejection is still a use, and *"is anyone holding this token?"* deserves a true answer. Enforcement lives in the auth middleware off the same lookup row expiry uses — no route lists to keep in sync. UI: a scope selector in the mint form (full / read-only) and a `read-only` marker in the list. Verified by the wire: read token GETs API and `/metrics` (200) → every write 403 naming the reason → restart → scope persisted and the refused write's stamp with it → `"full"` collapses to `null` → full token mints and revokes freely → `"admin"` 400.
- [x] **v1.31.0** — **Token network binding**. The credential-hygiene trilogy, closed: expiry bounds a token in **time** (v1.29), scope bounds its **powers** (v1.30), this bounds it in **place** — `POST /api/tokens` grows an optional `bound_cidr` (an IPv4 CIDR, or a bare address stored as `/32`): the token only opens the door when presented **from inside that network**, so the Prometheus token lifted from a scrape config is worthless to whoever holds it anywhere else. Absent stays `NULL` — bound nowhere, valid from anywhere, every earlier token unchanged (the third application of the soft-`ALTER TABLE` recipe). The interesting decision **inverts v1.30's**: scope refuses with a *named 403* because the caller proved identity — but a binding is an **anti-theft** control, and off its network the token must be indistinguishable from garbage: the **bare 401**, no error naming the binding, and **no `last_used_at` stamp** a thief could trip (`"never used"` next to a bound token means exactly that). The owner's answer to *"why did my relocated cron break"* is the binding shown in the token list. The comparison runs against the **socket's peer address** — IPv4-mapped IPv6 (`::ffff:a.b.c.d`, what a dual-stack listener reports for every v4 client) is unmapped, `::1` counts as loopback, and a genuinely-IPv6 peer never matches a v4 CIDR (the mint refuses IPv6 CIDRs up front rather than selling a binding that would silently never match; anything unparseable **fails closed**). Deliberately the *proxy's* address behind a reverse proxy — a forwarded header is attacker-writable, and a binding that trusted it would be theatre. Composes with scope and expiry off the same lookup row: one middleware, no route lists. UI: an optional CIDR field in the mint form and a `bound to 192.168.1.0/24` marker in the list. Verified by the wire: a token bound to `127.0.0.0/8` GETs from loopback (200, use stamped) → one bound to `10.99.0.0/16` presented from loopback gets the bare `Authentication required` 401 and its `last_used_at` stays `NULL` → an IPv6 CIDR at mint is a named 400.
- [x] **v1.32.0** — **Version-detection intensity**. Back to the roadmap's north star (expose an nmap option, additively): `-sV` names the product and version behind an open port, and how hard it tries is the intensity. A *Version detection* selector in *Advanced options* offers three levels that map 1:1 to nmap's own knob — **Light** (`--version-light`, the fast probes only), **Standard** (bare `-sV`, nmap's default intensity), **All** (`--version-all`, every probe, slowest but the one that pins an odd service on a non-standard port) — instead of a free-form 0-9 integer that would need its own range validation. The historical hardcoded `--version-light` becomes the *absent* default, so **every pre-v1.32 scan is byte-identical** (`validateVersionDetection(absent) → ['--version-light']`); the flag is spliced into both the TCP and UDP argv where the literal used to sit. Server-side validation rejects anything outside the three levels before it reaches `execFile`, the same allowlist discipline as scan-technique and NSE categories. Applies to both flows; the UDP scan reuses it. Unit-tested at the validator (absent/light/standard/all + six rejected shapes), 184 → 191 → **192 tests**.
...
For scripts and cron jobs, don't reuse the admin credential: mint an **API token** in the sidebar (or `POST /api/tokens`) and call the API with `Authorization: Bearer lsk_…` instead — revocable one by one, without ever rotating the admin password (v1.25.0). Since v1.29.0 a token can carry an expiry (`expires_in_days`): it stops opening the door at its deadline with the same bare 401 as a revoked one, but stays in the list, visibly expired — so a cron that breaks at 3 a.m. has its answer waiting in the sidebar. And since v1.30.0 a token can be **read-only** (`scope: "read"`): it can GET everything but change nothing — a write gets a 403 that names the reason. Give full access only to scripts that actually mutate. Since v1.31.0 a token can also be **bound to a network** (`bound_cidr: "192.168.1.0/24"`): presented from anywhere else it gets the same bare 401 as an invalid one — a stolen credential is worthless off your LAN.

### Can Prometheus / Grafana scrape it?

Yes — `GET /metrics` serves the Prometheus text format with per-network host counts, last-scan clocks, pending alerts by type, and schedule totals (v1.28.0). With auth enabled, mint a **read-only** API token for the scraper (scope *Read-only* in the sidebar, or `"scope": "read"` in the POST — v1.30.0) and put it in the scrape config instead of the admin password. A scraper only ever asks, so if that credential leaks it can look but never launch a scan:
